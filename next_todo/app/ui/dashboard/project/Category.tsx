  ...
  return (
    <div className="">
      ...
      <ul className="mt-4 flex gap-4 flex-wrap">
        <li
          className={`mb-2 p-2 bg-gray-100 rounded hover:bg-gray-200 cursor-pointer ${categoryId === undefined  ? 'bg-orange-700 font-bold text-white' : ''}`} // 選択中のカテゴリーは色を変える
          onClick={() => handleClick('all')} // クリックで全てのチェックリストに遷移する処理を追加
        >すべて</li>
        {categories.map((category) => (
          <li key={category.id} 
          className={`mb-2 p-2 bg-gray-100 rounded hover:bg-gray-200 cursor-pointer ${category.id === categoryId ? 'bg-orange-700 font-bold text-white' : ''}`} // 選択中のカテゴリーは色を変える
